...
async fn test_vector_upsert_and_search() {
    ...
    stores
        .vectors
        .upsert(id, vector.clone(), serde_json::json!({"tenant_id": "t1"}))
        .await
        .unwrap();
    let results = stores.vectors.search(vector, "t1", 10, None).await.unwrap();
...
async fn test_auth_create_and_validate() {
    ...
    let key = stores
        .auth
        .create_api_key("t1", vec!["read".into(), "write".into()])
        .await
        .unwrap();
...
async fn test_conflict_resolution_flow() {
    ...
    let mut conflict = ConflictRecord::new("t1", c1.id, c2.id, ConflictType::DirectContradiction);
    ...
    assert_eq!(
        conflicts[0].resolution_note.as_deref(),
        Some("Both claims valid in different contexts")
    );
}
...
async fn test_ingest_store_query_flow() {
    ...
    stores
        ...
        .upsert(
            claim1.id,
            v1.clone(),
            serde_json::json!({"tenant_id": "t1", "content": &claim1.content}),
        )
        .await
        .unwrap();
    stores
        ...
        .upsert(
            claim2.id,
            v2.clone(),
            serde_json::json!({"tenant_id": "t1", "content": &claim2.content}),
        )
        .await
        .unwrap();
    ...
    stores
        .graph
        .add_edge(claim1.id, claim2.id, "RELATED_TO", 0.9)
        .await
        .unwrap();
