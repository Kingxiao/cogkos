...
fn make_claim(content: &str, tenant: &str) -> EpistemicClaim {
    EpistemicClaim::new(
        ...
        AccessEnvelope::new(tenant), ProvenanceRecord::new("test".into(), "test".into(), "test".into()),
    )
}
...
async fn test_tenant_isolation_on_query() {
    ...
    store.insert_claim(&make_claim("A1", "tenant-a")).await.unwrap();
    store.insert_claim(&make_claim("A2", "tenant-a")).await.unwrap();
    store.insert_claim(&make_claim("B1", "tenant-b")).await.unwrap();
...
async fn test_auth_key_per_tenant() {
    ...
    let key_a = store.create_api_key("tenant-a", vec!["read".into()]).await.unwrap();
    let key_b = store.create_api_key("tenant-b", vec!["write".into()]).await.unwrap();
