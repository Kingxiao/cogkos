...
impl crate::GraphStore for InMemoryGraphStore {
    ...
    async fn activation_diffusion(
        ...
        while let Some((current_id, current_depth)) = queue.pop() {
            ...
            // Find all edges from/to current node and propagate activation
            for (from, to, relation, custom_weight) in edges.iter() {
                ...
                // Non-propagating relations: never spread activation regardless of custom_weight
                let default_weight = get_relation_weight(relation);
