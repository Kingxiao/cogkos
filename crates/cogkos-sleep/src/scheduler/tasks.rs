...
use crate::content_consolidation::{extract_speaker_facts, group_claims_by_speaker};
...
/// Content consolidation: aggregate episodic turns into semantic facts.
///
/// For each tenant, groups episodic claims by speaker entity, extracts activity /
/// relationship / career facts via keyword matching, deduplicates against existing
/// semantic claims, and creates new semantic-layer claims with `derived_from` links.
pub(crate) async fn run_content_consolidation(stores: &Arc<Stores>) -> Result<usize> {
    let start = std::time::Instant::now();
    info!("Running content consolidation");

    let tenants = match stores.claims.list_tenants().await {
        Ok(t) if !t.is_empty() => t,
        Ok(_) => vec!["default".to_string()],
        Err(e) => {
            warn!("Failed to list tenants for content consolidation: {}", e);
            vec!["default".to_string()]
        }
    };
    ...
    for tenant_id in &tenants {
        // 1. Get all episodic claims for this tenant
        let episodic_claims = stores
            .memory_layers
            .list_claims_by_memory_layer(tenant_id, "episodic", None, 5000)
            .await
            .unwrap_or_default();

        if episodic_claims.len() < 10 {
            debug!(
                tenant = %tenant_id,
                count = episodic_claims.len(),
                "Not enough episodic claims to consolidate (need >= 10)"
            );
            continue;
        }

        // 2. Group by speaker
        let groups = group_claims_by_speaker(&episodic_claims);

        for (speaker, speaker_claims) in &groups {
            if speaker_claims.len() < 3 {
                continue; // Need at least 3 turns to consolidate
            }

            // 3. Extract facts
            let facts = extract_speaker_facts(speaker, speaker_claims);

            for fact in &facts {
                // 4. Check if similar semantic claim already exists
                let existing = stores
                    .claims
                    .search_claims(tenant_id, &fact.content, 3)
                    .await
                    .unwrap_or_default();

                let already_exists = existing.iter().any(|c| {
                    c.metadata.get("memory_layer").and_then(|v| v.as_str()) == Some("semantic")
                        && c.content.to_lowercase().contains(&speaker.to_lowercase())
                        && c.metadata.get("consolidated_from").is_some()
                });
                ...
                if already_exists {
                    debug!(
                        ...
                        "Skipping — consolidated fact already exists"
                    );
                    continue;
                }

                // 5. Create new semantic claim
                ...
                metadata.insert(
                    ...
                    serde_json::Value::String("content_consolidation".to_string()),
                );
                metadata.insert(
                    "source_speaker".to_string(),
                    serde_json::Value::String(speaker.clone()),
                );
                metadata.insert(
                    "source_count".to_string(),
                    serde_json::Value::Number(serde_json::Number::from(
                        fact.source_claim_ids.len(),
                    )),
                );

                let now = chrono::Utc::now();
                let consolidated = cogkos_core::models::EpistemicClaim {
                    id: uuid::Uuid::new_v4(),
                    tenant_id: tenant_id.clone(),
                    content: fact.content.clone(),
                    node_type: cogkos_core::models::NodeType::Insight,
                    knowledge_type: cogkos_core::models::KnowledgeType::Experiential,
                    structured_content: None,
                    claimant: cogkos_core::models::Claimant::System,
                    epistemic_status: cogkos_core::models::EpistemicStatus::Corroborated,
                    confidence: 0.75,
                    consolidation_stage: cogkos_core::models::ConsolidationStage::Consolidated,
                    version: 1,
                    durability: 0.8,
                    ...
                    t_valid_end: None,
                    t_known: now,
                    access_envelope: cogkos_core::models::AccessEnvelope::new(tenant_id),
                    provenance: cogkos_core::models::ProvenanceRecord::new(
                        format!("content-consolidation-{}", speaker),
                        "system".to_string(),
                        "sleep-time-consolidation".to_string(),
                    ),
                    vector_id: None,
                    last_prediction_error: None,
                    derived_from: fact.source_claim_ids.clone(),
                    superseded_by: None,
                    entity_refs: Vec::new(),
                ...
                if stores.claims.insert_claim(&consolidated).await.is_ok() {
                    // Embedding will be generated lazily on first query hit,
                    // or by a dedicated embedding backfill task.
                    // The sleep scheduler does not hold an embedding client.
                    ...
                    info!(
                        ...
                        "Created consolidated semantic fact"
                    );
                }
            }
        }
