//! Phase 3 periodic task spawn implementations (paradigm shift, framework health, collective wisdom, LLM extraction)
...
use crate::llm_extraction::run_llm_extraction;
...
impl Scheduler {
    ...
    pub(crate) fn spawn_llm_extraction(&self) {
        // Guard: only spawn if LLM client is available and feature is enabled
        let enabled = std::env::var("LLM_EXTRACTION_ENABLED")
            .map(|v| v == "true" || v == "1")
            .unwrap_or(false);
        if !enabled {
            info!("LLM extraction disabled (set LLM_EXTRACTION_ENABLED=true to enable)");
            return;
        }
        let llm = match &self.llm_client {
            Some(c) => c.clone(),
            None => {
                warn!("LLM extraction enabled but no LLM client configured — skipping");
                return;
            }
        };
        ...
        let s = self.clone_instance();
        tokio::spawn(async move {
            let mut ticker = interval(Duration::from_secs(s.config.llm_extraction_interval_secs));
            let mut failures: u32 = 0;
            loop {
                tokio::select! {
                    _ = s.cancel.cancelled() => { info!("LLM extraction task shutting down"); break; }
                    _ = ticker.tick() => {}
                }
                if failures >= MAX_FAILURES {
                    let backoff = compute_backoff(s.config.llm_extraction_interval_secs, failures);
                    warn!(
                        task = "llm_extraction",
                        failures, "Circuit breaker: backing off"
                    );
                ...
                if !s.check_budget(TaskType::LlmExtraction, 100).await {
                    continue;
                }
                let start_time = std::time::Instant::now();
                match run_llm_extraction(&s.stores, llm.as_ref()).await {
                    Ok(count) => {
                        failures = 0;
                        s.record_processed(TaskType::LlmExtraction, count as u64)
                            .await;
                        s.state.write().await.last_llm_extraction = Some(chrono::Utc::now());
                        if count > 0 {
                            info!(count, "LLM extraction created new semantic facts");
                        }
                    }
                    Err(e) => {
                        failures += 1;
                        error!(task = "llm_extraction", error = %e, failures, "Task failed");
                        s.state.write().await.errors += 1;
                    }
                }
                check_time_budget(start_time, s.config.max_task_duration_ms, "LLM extraction");
            }
        });
    }
}
