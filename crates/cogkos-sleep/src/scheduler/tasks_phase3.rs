    ...
    for tenant in &tenants {
        ...
        if result.recommendation == AnomalyRecommendation::InitiateParadigmShift {
            warn!(
                tenant = %tenant,
                anomaly_count = result.anomalies.len(),
                "Paradigm shift recommended — persisting event + webhook notification"
            );

            // Persist as system event claim (queryable via MCP)
            let anomaly_summary: Vec<String> = result.anomalies.iter()
                .map(|a| format!("{:?}: {}", a.anomaly_type, a.description))
                .collect();
            let content = format!(
                "Paradigm shift recommended: anomaly_score={:.2}, anomalies: {}",
                result.anomaly_score,
                anomaly_summary.join("; ")
            );
            let mut claim = cogkos_core::models::EpistemicClaim::new(
                content,
                tenant.clone(),
                cogkos_core::models::NodeType::Event,
                cogkos_core::models::Claimant::System,
                cogkos_core::models::AccessEnvelope::new(tenant),
                cogkos_core::models::ProvenanceRecord::new(
                    "cogkos-scheduler".to_string(),
                    "system".to_string(),
                    "paradigm_shift_detection".to_string(),
                ),
            );
            claim.confidence = result.anomaly_score;

            if let Err(e) = stores.claims.insert_claim(&claim).await {
                warn!(tenant = %tenant, error = %e, "Failed to persist paradigm shift event");
            }

            // Webhook notification (reuses CONFLICT_WEBHOOK_URL)
            notify_paradigm_shift(tenant, result.anomaly_score, result.anomalies.len());
        }
...
/// Fire-and-forget webhook notification for paradigm shift events.
fn notify_paradigm_shift(tenant_id: &str, anomaly_score: f64, anomaly_count: usize) {
    if let Ok(url) = std::env::var("CONFLICT_WEBHOOK_URL") {
        let payload = serde_json::json!({
            "event": "paradigm_shift_recommended",
            "tenant_id": tenant_id,
        ...
        tokio::spawn(async move {
            ...
            if let Err(e) = client
                ...
                .await
            {
                warn!(error = %e, "Failed to send paradigm shift webhook");
            }
        });
    }
}
