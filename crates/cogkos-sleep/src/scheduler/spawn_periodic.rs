...
use super::tasks::{
    run_confidence_boost, run_consolidation, run_content_consolidation, run_decay,
    run_health_check, run_insight_extraction, run_memory_gc, run_memory_promotion,
    run_pending_aggregation, run_prediction_validation,
};
...
impl Scheduler {
    ...
    pub(crate) fn spawn_periodic_tasks(&self) {
        ...
        self.spawn_content_consolidation();
        self.spawn_llm_extraction();
    }
    ...
    fn spawn_content_consolidation(&self) {
        let s = self.clone_instance();
        tokio::spawn(async move {
            let mut ticker = interval(Duration::from_secs(
                s.config.content_consolidation_interval_secs,
            ));
            let mut failures: u32 = 0;
            loop {
                tokio::select! {
                    _ = s.cancel.cancelled() => { info!("Content consolidation task shutting down"); break; }
                    _ = ticker.tick() => {}
                }
                if failures >= MAX_FAILURES {
                    let backoff =
                        compute_backoff(s.config.content_consolidation_interval_secs, failures);
                    warn!(
                        task = "content_consolidation",
                        failures, "Circuit breaker: backing off"
                    );
                    tokio::time::sleep(backoff).await;
                }
                if !s.check_budget(TaskType::ContentConsolidation, 100).await {
                    continue;
                }
                let start_time = std::time::Instant::now();
                match run_content_consolidation(&s.stores).await {
                    Ok(count) => {
                        failures = 0;
                        s.record_processed(TaskType::ContentConsolidation, count as u64)
                            .await;
                        s.state.write().await.last_content_consolidation = Some(chrono::Utc::now());
                        if count > 0 {
                            info!(count, "Content consolidation created new semantic facts");
                        }
                    }
                    Err(e) => {
                        failures += 1;
                        error!(task = "content_consolidation", error = %e, failures, "Task failed");
                        s.state.write().await.errors += 1;
                    }
                }
                check_time_budget(
