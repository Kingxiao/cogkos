...
impl From<&ConflictRecord> for ConflictSummary {
    fn from(conflict: &ConflictRecord) -> Self {
        // Extract claim summaries from description format:
        // "ConflictType: [claim_a...] vs [claim_b...]"
        let (summary_a, summary_b) = conflict
            .description
            .as_deref()
            .and_then(|desc| {
                let start_a = desc.find('[')? + 1;
                let end_a = desc.find("...] vs [")?;
                let start_b = end_a + 8; // len of "...] vs ["
                let end_b = desc.rfind("...]")?;
                Some((
                    desc[start_a..end_a].to_string(),
                    desc[start_b..end_b].to_string(),
                ))
            })
            .unwrap_or_else(|| {
                // Fallback: use description as-is for claim_a, empty for claim_b
                let desc = conflict.description.clone().unwrap_or_default();
                (desc, String::new())
            });
