...
use crate::aggregation::{AggregatedResponse, AggregationMetadata, FederatedResult, NodeResult, ResultAggregator, WeightedAggregator};
use crate::error::{FederationError, Result};
use crate::node::{FederatedNode, NodeRegistry};
use crate::routing::{FederatedQuery, NodeRoute, QueryPriority, QueryRouter, RoutingDecision, RoutingStrategy};
...
impl CrossInstanceQueryExecutor {
    ...
    pub async fn execute(
        ...
        query: FederatedQuery,
        registry: &NodeRegistry,
    ) -> Result<crate::aggregation::FederatedResult> {
        ...
        // Aggregate results
        let aggregator = crate::aggregation::WeightedAggregator::new();
        
        let node_results: Vec<crate::aggregation::NodeResult> = results
            .into_iter()
            .map(|r| r.unwrap_or_else(|e| {
                crate::aggregation::NodeResult {
        ...
        Ok(crate::aggregation::FederatedResult {
            query_id: query.query_id.clone(),
            node_results,
            aggregated,
            metadata: crate::aggregation::AggregationMetadata {
    ...
    async fn execute_node_query(
        ...
        route: &NodeRoute,
        query: &FederatedQuery,
    ) -> Result<crate::aggregation::NodeResult> {
        ...
        let response = self
            ...
            .send()
            .await
            .map_err(|e| FederationError::NetworkError(e))?;
        ...
        if response.status().is_success() {
            ...
            Ok(crate::aggregation::NodeResult {
...
impl CrossInstanceRouterBuilder {
    ...
    pub fn build(self) -> CrossInstanceRouter {
        let metadata_directory = self.metadata_directory.unwrap_or_else(MetadataDirectory::new);
