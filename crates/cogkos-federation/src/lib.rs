...
//!
//! Cross-instance knowledge federation with collective intelligence.
//! All modules active: routing, cross-instance queries, health monitoring, aggregation.
...
pub use cross_instance::{
    CrossInstanceRouter, CrossInstanceQueryExecutor, CrossInstanceRouterBuilder,
    MetadataDirectory, DomainExpertise, QueryAnalyzer,
};
...
impl FederationClientBuilder {
    ...
    pub fn build(self) -> Result<FederationClient> {
        ...
        // If metadata directory provided, use CrossInstanceRouter; otherwise DomainRouter
        let router: Arc<dyn QueryRouter> = if let Some(dir) = self.metadata_directory {
            Arc::new(CrossInstanceRouter::new(dir))
        } else {
            self.router
