...
fn build_state(
    ...
    // Federation client (optional, enabled via FEDERATION_ENABLED env var)
    let federation_client = if std::env::var("FEDERATION_ENABLED")
        .map(|v| v == "true" || v == "1")
        .unwrap_or(false)
    {
        match cogkos_federation::FederationClientBuilder::new().build() {
            Ok(client) => {
                tracing::info!("Federation layer: enabled");
                Some(Arc::new(client))
            }
            Err(e) => {
                tracing::warn!("Federation layer: failed to initialize: {}", e);
                None
            }
        }
    } else {
        tracing::debug!("Federation layer: disabled (set FEDERATION_ENABLED=true to enable)");
        None
    };
