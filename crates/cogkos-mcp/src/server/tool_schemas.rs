...
pub fn build_tools() -> Vec<Tool> {
    ...
    // federation_register_node
    let mut input_schema = JsonObject::new();
    input_schema.insert("type".to_string(), serde_json::json!("object"));
    input_schema.insert("properties".to_string(), serde_json::json!({
        "node_id": {"type": "string", "description": "Unique identifier for the remote CogKOS instance"},
        "name": {"type": "string", "description": "Human-readable name"},
        "endpoint": {"type": "string", "description": "MCP endpoint URL (e.g. http://host:3000/mcp)"},
        "domains": {"type": "array", "items": {"type": "string"}, "description": "Knowledge domains this node specializes in"},
        "expertise": {"type": "object", "additionalProperties": {"type": "number"}, "description": "Domain expertise scores (0.0-1.0)"}
    }));
    input_schema.insert(
        "required".to_string(),
        serde_json::json!(["node_id", "name", "endpoint"]),
    );
    inject_api_key(&mut input_schema);
    tools.push(Tool::new(
        "federation_register_node",
        "Register a remote CogKOS instance for federated queries",
        input_schema,
    ));
    ...
    input_schema.insert("type".to_string(), serde_json::json!("object"));
    input_schema.insert("properties".to_string(), serde_json::json!({
        "query": {"type": "string", "description": "Query text to search across federated instances"},
        "domains": {"type": "array", "items": {"type": "string"}, "description": "Limit query to specific domains"},
        "timeout_ms": {"type": "integer", "description": "Query timeout in milliseconds (default 30000)"},
        "priority": {"type": "string", "enum": ["low", "normal", "high", "critical"], "description": "Query priority"}
    }));
    input_schema.insert("required".to_string(), serde_json::json!(["query"]));
    inject_api_key(&mut input_schema);
    tools.push(Tool::new(
        "federation_query",
        "Query knowledge across federated CogKOS instances",
        input_schema,
    ));
    ...
    tools.push(Tool::new(
        "federation_health",
        "Check federation network health and registered nodes",
        input_schema,
    ));

    // federation_export_insights
    let mut input_schema = JsonObject::new();
    input_schema.insert("type".to_string(), serde_json::json!("object"));
    input_schema.insert("properties".to_string(), serde_json::json!({
    ...
    tools.push(Tool::new(
        "federation_export_insights",
        "Export anonymized knowledge insights for cross-instance sharing",
        input_schema,
    ));

    // ── System self-reflection tools ─────────────────────────────
    ...
    tools.push(Tool::new(
        "system_health",
        "Framework health snapshot: prediction accuracy, conflict resolution, knowledge diversity, insight generation rate",
        input_schema,
    ));
    ...
    tools.push(Tool::new(
        "evolution_status",
        "Paradigm shift detection status: anomaly score, recommendation, detected anomalies",
        input_schema,
    ));
    ...
    tools.push(Tool::new(
        "system_introspection",
        "Multi-tenant system overview: health comparison across all tenants",
        input_schema,
    ));
